...
//
// 有効化・無効化は docs/jev.md の「Jev plan gate」節を参照。要約:
// 環境変数 JEV_API_KEY があれば最優先、無ければこのマシンの
// `~/dotfiles/.env` を読む。どちらも無ければ完全な no-op。実行時に呼び出しが
// 失敗した場合もそのセッション内では以後リトライせず静かに no-op へ切り替える。
//
