...
前回のセッションを同じ端末で続ける場合は次を使います。
日をまたいだセッションや、長時間放置した大きなセッションは継続せず、後述の `/handoff` で新しいセッションへ移ってください。
...
| `/handoff [引き継ぎの焦点]` | 現在の文脈から引き継ぎを生成し、新しいセッションへ移る |
| `/resume` | 保存済みセッションへ切り替える |
| `/new` | 空の新規セッションを開始する |
| `/fork` | 現在の会話から別のセッションを分岐する |
| `/clear` | セッション ID と記録を残したまま、現在の会話コンテキストを空にする |
| `/skill:名前` | 指定した Skill の手順を読み込んで実行する |

`/new` と `/clear` は引き継ぎを生成しません。
判断、未完了作業、失敗した試行を次のセッションへ渡す必要がある場合は `/handoff` を使います。
...
次の場合は、現在のセッションを無理に延命せず `/handoff` を使います。

- 日をまたいで作業を再開する
- 一時間以上放置した大きなセッションへ戻る
- `/context` でコンテキストが大きく、圧縮を繰り返している
- 問題の焦点が変わり、過去の試行が判断を邪魔している

```text
/handoff 残っている不具合、変更済みファイル、未実行の確認に絞って引き継いで
```

`/handoff` は引き継ぎを作るだけで終了せず、新しいセッションへ切り替えます。
