...
# that merely mentions agmsg is not blocked. The automatic read-only
# check-inbox Stop hook is stopped with exit 0 instead of a noisy hook error;
# its agmsg body still never runs.
#
...
if [ "${HERDR_ENV-}" = 1 ] && [ "${HERDR_AGMSG_ALLOW-}" != 1 ]; then
  ...
  _hcg_silent=""
  case "${0-}" in
    */agmsg/scripts/check-inbox.sh) _hcg_hit=1; _hcg_silent=1 ;;
  ...
  if [ -n "$_hcg_hit" ]; then
    if [ -n "$_hcg_silent" ]; then
      unset _hcg_hit _hcg_silent _hcg_str
      exit 0
    fi
