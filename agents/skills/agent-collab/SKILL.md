---
name: agent-collab
description: agmsg を使ったエージェント間ペアフロー（レビュー往復・タスク受け渡し・調査共有）の手順とメッセージテンプレ。ペアレビューを始めるとき、agmsg でレビュー依頼・受け渡し・共有をするとき、[REVIEW-REQ] 等のタグ付きメッセージを受信したときに使用する。不変条件（trust boundary、レビュアーはツリーを編集しない等）は global-instructions の「Agent collaboration」節が正本で、このスキルは手順とテンプレだけを持つ。
---
...
（team=リポ名、impl=claude-code / reviewer=codex、`--with-copilot` で
copilot 追加。delivery mode も標準値まで設定される）。手動 join は
役割名やモードがぶれるので使わない。
...
1. **spawn**（第一選択、claude-code / codex のみ）:
   `/agmsg spawn codex reviewer`（Claude から）/
   `$agmsg spawn claude-code impl`（Codex から）。join・actas 済みで
   起動し、ready まで待つ。tmux 内ならペイン、外ならターミナルの
   新規ウィンドウが開く。**codex を spawn するときは必ず
   `--boot-prompt` で「inbox を確認して対応せよ」まで指示する** —
   codex は Monitor を持たず、boot 後はアイドルに戻るため、
   これがないと送信済みメッセージに気づかない。
